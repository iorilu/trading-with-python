...
def tstat(x,a,b,M):
    ...
    for j in range(a):
        ...
        if(j==0):
            ...
            y=np.vstack((y,hwt))
    print("  ...completed")
    print(("Computation of t-statistics..."), end=' ')
    t=[];
    ...
    t=np.array(t)
    print("completed")
    print(("Computation of p-values..."), end=' ')
...
def overalltest(cl,otest):
    ...
    if(np.mean(otest)>=99.0):
        print(("   Signal x(t) displays an evidence for RANDOMNESS"), end=' ')
        T=1
    else:
        print(("   Signal x(t) displays an evidence for NON-RANDOMNESS"), end=' ')
        T=0
